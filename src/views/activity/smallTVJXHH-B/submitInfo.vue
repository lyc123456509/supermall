<template>
  <div class="container">
    <img src="@/assets/activity/smallTV/img_gxcz.png" alt="" style="width: 6.77rem; margin-top: 0.54rem" v-if="!isFromOrder" />
    <div class="product">
...
<script lang="ts" setup>
  ...
  const isFromOrder = ref(router.currentRoute.value.query.origin);

  const submit = () => {
    ruleForm.value.validate().then(({ valid, errors }: any) => {
      if (valid) {
        tvStore.setUser({ ...formData, phone: formData.phone.replace(/\s*/g, '') });
        router.push('/activity/smallTVJXHH-B/payEms');
      } else {
  ...
  //  自动填充
  onMounted(() => {
    formData.reciver = tvStore.user.reciver;
    formData.phone = tvStore.user.phone;
    formData.areaCode = tvStore.user.areaCode?.split('-') || [];
    formData.address = tvStore.user.address;
  });
