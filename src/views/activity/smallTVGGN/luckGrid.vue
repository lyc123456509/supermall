...
<script lang="ts" setup>
  ...
  const startCallback = async () => {
    ...
    const res = await raffleprobability({ code: sessionStorage.getItem('openId'), type: 2 }, businessDomain['xjc'][import.meta.env.MODE]);
    Toast.hide('loading');
    const content = res.data.value.content;
    if (content) {
      if (!content.luckDraw) {
    ...
    setTimeout(() => {
      ...
      setTimeout(() => {
        router.push('/activity/smallTVGGN/submitInfo');
      }, 4000);
    }, 3000);
  };
