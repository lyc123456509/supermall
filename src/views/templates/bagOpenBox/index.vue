<template>
  <div>
    <div class="wp-landing30 pro4">
      ...
      <!-- 必中商品展示 -->
      <div class="box-contain-pro">
        <div class="title">该盲盒必中以下商品之一</div>
        <div class="main">
          <div class="pro-list _flex">
            <div class="pic _flex_cc" :class="index === 0 ? 'cur' : ''" v-for="(item, index) in swiperList.slice(0, 6)" :key="index"
              ><img :src="item.imgUrl"
            /></div>
          </div>
          <div class="check-more-btn _flex_cc"><span class="text" @click="openGoodsDetail">点击查看所有商品</span></div>
        </div>
      </div>
      <!-- 商品标题 -->
...
<script setup lang="ts">
  ...
  import { swiperList, shopRule, shopRule2 } from './goodsList';
