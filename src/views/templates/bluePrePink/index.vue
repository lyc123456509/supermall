<template>
  <prePink v-if="step === 1" @change-step="changeStep" />
  <div v-else>
    <div class="container">
      <div class="header">
        ...
        <span style="font-size: 0.62rem; margin: 0.4rem 0 0.3rem 0; font-weight: bold">锁定百元话费券！</span>
        <nut-countdown :endTime="end" v-model="resetTime" millisecond format="HH:mm:ss:SS" class="">
      ...
      <div class="content">
        <nut-input
          type="tel"
          v-model="phone"
          placeholder="输入手机号领话费"
          maxLength="11"
          :border="false"
          :right-icon="phoneIcon"
          style="width: 100%; background: transparent; font-size: 0.45rem; color: #333; font-weight: bold; padding: 0 0.24rem"
        />
        <p class="wait-save"
        ...
        <div class="sw">
          <img src="https://html.sccdyzl.com/vue-cps/blue/icon_yhq.png" alt="" />
        ...
        <div class="pay_amount">
          ...
          <span class="value">{{ info.phoneData.cssClass }}元</span>
        </div>
        ...
        <div class="buttonBox scale-animation">
          ...
          <nut-button :loading="submitLoading" class="nowGet" size="large" @click="getPayHandle">立即支付</nut-button>
        </div>
...
<script setup lang="ts">
  import prePink from '/@/views/prePage/prePink/index.vue';
  ...
  const { phone, step, resetTime, end } = toRefs(state);
</script>
...
<style lang="scss" scope>
  ...
  .part-item {
    ...
    width: 0.45rem;
    height: 0.45rem;
    background: -webkit-linear-gradient(top, #fdfffa, #fef0b9);
    background: linear-gradient(180deg, #fdfffa, #fef0b9);
  ...
  .container {
    background-image: url(https://html.sccdyzl.com/vue-cps/blue/bg_ls.png);
    width: 100vw;
    min-height: 100vh;
