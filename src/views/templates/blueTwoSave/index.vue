  ...
  <div>
    <div class="container">
      ...
      <div class="content">
        ...
        <div class="download_btn" @click="downloadTo">
          <p>下载APP</p>
          <p style="color: #eee; font-size: 0.18rem">(成为平台会员)</p>
        </div>
      </div>
      <div class="hf_c_box">
...
<script setup lang="ts">
  ...
  const downloadTo = async (): Promise<void> => {
    let res = await getDownloadLink(info.value.applyIdentification, { pageType: 1 });
    let link = '';
    link = res?.data.value!.content!.fileUrl || res?.data.value!.fileUrl;
    window.open(link);
  };
